...
while (!quitFlag){
	...
	HighBandDenom = 25  # Initial value should be 15 to make a 1/15 highband cutoff
...
for (oy in (oystercode) ){ 
  ...
  write.csv(dfheart,
  		file = paste0(heartoutputpath,'bpmQA.csv'),  
  		row.names=FALSE)
}
```
...
allhrmeta<-merge(bpmQA,metafile[, c("Site", "Treatment","Alive","Code")], by="Code")

ggplot(allhrmeta, aes(DateTime, finalBPM, color=Site))+
