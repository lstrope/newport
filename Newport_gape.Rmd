...
#halldata = gapedat
#metadf = metafile
exciseHall = function(halldata, metadf, maintenance, maxGapFill = 12) {
	...
	# weren't available
		#######################################################
	# TODO: Go to the low tide servicing metadata file and get the relevant
...
for (oy in (oystercode) ){ 
  ...
  if (exists('combineddat')){
    if (nrow(combineddat) > 0){
      ...
      gapedat = exciseHall(halldata = gapedat, metadf = metafile, maintenance = maintenance)
...
for(oy in (oystercode)){
  ...
  gapedat=read.csv(gapepath)
	...
	gapedat[,"hallpercent"] = calcPercentGapeNLMEsubset(gapedat[,"Hall"], cRegress, percentileLim = c(0.01,0.95), maxrow=1440*6)
...
allgape$DateTime=as.POSIXct(allgape$DateTime)
...
#attempt at for loop to do this
# serialnumber=unique(metafile[,1])
# j=loadComboGape(SN165)
# for (i in serialnumber) {
#   if (!exists(paste0(combogapefile,i,'_combogape.csv')))
#     continue;
#   j=loadComboGape(i)
#   plot2HallFunction(j)
#   png("SN130.png")
