...
ggplot(weightedgapebinary, aes(Treatment, prop_open, fill = Treatment)) + # ggplot2 barplot with error bars
...
ggplot(weightedgapebinary, aes(Site, prop_open))+
  #geom_bar(position="dodge", stat="summary", fun="mean", fill=c("#999999", "#009E73", "chocolate4"), col="black")+
...
seasondata=season %>% 
  mutate(set = recode(set, '1' = 'Wet', '2' = 'Dry')) %>% 
  select(-c(binary))

colour_palette = c("#C9C9C9", "#999999", "#70DCBE", "#009E73", "#DC9867", "chocolate4")
...
avgseasontreatment=seasondata %>% 
  group_by(Treatment,season) %>% 
   mutate(avgpercentopen=mean(percentopen),se=sd(percentopen)/sqrt(3))

ggplot(avgseasontreatment, aes(Treatment, avgpercentopen,fill=interaction(season,Treatment))) +
  geom_col_pattern(
    aes(pattern = season),
    colour = "black",
    pattern_fill = "black",
...
ggsave(path=plots, "gape_seasontreatment.png", width=6, height=6)

#####SITE###############
avgseasonsite=seasondata %>% 
  group_by(Site,season) %>% 
   mutate(avgpercentopen=mean(percentopen),se=sd(percentopen)/sqrt(4))
...
ggsave(path=plots, "gape_seasonSite.png", width=6, height=6)
...
ggsave(path=plots, "Temp vs treatments.png",width=6, height=6)
