...
gapBounds2=function(mygaps, maxrow){
  ...
  if (mydiff>maxrow){
    ...
    for (i in 1:nloops){
      if (i == 1){
        mygaps2 = data.frame(Start = mygaps$Start[1], End = maxrow)
...
# These files tend to have a comma after the last column's data, which makes read.csv think there's one more column 
# of data than there are names in the header row, which then treats the 1st column of POSIXt values as row numbers, 
# which is unfortunate. You need to handle that appropriately. 
...
# for loop that says go through each serial number in the vector 'serialnumber' and find all files, 
...
# TODO: Recalibrate with BivalveBit boards
# TODO: Modify calcPercentGapeNLME to account for shifting baseline in the Hall signal as the animal grows 
...
for(i in (serialnumber)){
  ...
  gapedat=read.csv(gapepath)
	...
	gapedat[,"hallpercent"] = calcPercentGapeNLMEsubset(gapedat[,"Hall"], cRegress, 
			percentileLim = c(0.01,0.95), maxrow=1440*7) #1440=# min in a day, *7 days for one week of sampling
...
#subdivide SN by treatment/site and make plots of gape for each oyster. Import metadata to see which serial number goes to each treatment. 
```{r combine-all-gape-files-and-add-metadata}
library(readr)
library(dplyr)
#there's probably a better way to do this but set wd to folder w/ all gape files to be combined
#setwd(combogapefile)
#combine all data frames into 1
allgape <- list.files(path='~/Documents/Oyster/data/Newport_SSINP/combinedgape') %>% 
  lapply(read_csv) %>% 
  bind_rows 
#return wd
#setwd("~/Documents/Oyster/Rcode")
...
ggplot(subset(allgapemeta, Site %in% c("PCH")), aes(x = POSIXt, y = Hall, col=SN))+
  geom_point()
```
