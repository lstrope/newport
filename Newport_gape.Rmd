...
#Get max of percentilRange and use as raw hall value. Then plug in as argument in \# Do this with aVal and bVal

#Figure out why the combogape file has a bunch of extraneous columns added onto the front of the file. They may be rownumbers getting saved as new columns
...
#remove bad oysters from all gape if not manually removed from files
```{r}
...
#calculate proportion open/closed just for 2022
```{r}
...
ggsave(path=plots, "proportion_open_treatment_2023.png", width=6, height=6)
```
...
#plot field deaths by site or treatment
```{r}
yesdead<-metafile %>%
