...
#Get max of percentilRange and use as raw hall value. Then plug in as
argument in \# Do this with aVal and bVal

##Callibration stuff???

Figure out why the combogape file has a bunch of extraneous columns
added onto the front of the file. They may be rownumbers getting saved
as new columns
...
gapedat=gapedat %>% 
  ...
  filter(DateTime<"2023-05-02 20:00:00")
plot(gapedat$DateTime, gapedat$Hall) #check to make sure worked
...
#####S.M.2.1090 
gapepath=file.path(paste0(combogapefile, "S.M.2.1090", "_combogape.csv"))
gapedat=read.csv(gapepath)
gapedat$DateTime=as.POSIXct(gapedat$DateTime, tz="UTC", format="%Y-%m-%d %H:%M:%S")
gapedat=gapedat %>% 
  filter(DateTime>"2023-04-23 00:00:00")%>% 
  filter(DateTime<"2023-04-27 00:00:00")
plot(gapedat$DateTime, gapedat$Hall) #check to make sure worked
write.csv(gapedat, paste0(combogapefile, "S.M.2.1090", "_combogape.csv"), row.names=FALSE)


```

#double check with luke
```{r}
#####D.E.3.1036
...
gapedat$DateTime=as.POSIXct(gapedat$DateTime, tz="UTC", format="%Y-%m-%d %H:%M:%S")
gapedat=gapedat %>% 
  filter(DateTime>"2022-12-26 12:00:00")
...
gapebinaryopen=gapebinarypercent[gapebinarypercent$binary == '1', ]

cbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#0072B2")

ggplot(gapebinaryopen, aes(Treatment, percentopen, fill = Treatment)) +      # ggplot2 barplot with error bars
...
ggsave(path=plots, "proportion_open_treatment.png", width=7, height=6)

ggplot(gapebinaryopen, aes(Site, percentopen))+
  #geom_bar(position="dodge", stat="summary", fun="mean", fill=c("#999999", "#009E73", "chocolate4"), col="black")+
  geom_bar(position="dodge", stat="summary", fun="mean", fill=cbPalette)+
...
ggplot(gapebinarypercent, aes(Treatment, percentopen, fill=binary))+
  geom_bar(stat="identity", position="fill")+
  theme_classic()+
  scale_fill_discrete(labels=c('Open', 'Closed'), name="Valve gape")+
  labs(y="Valve gape")

ggplot(gapebinarypercent, aes(Site, n, fill=binary))+
  geom_bar(stat="identity", position="fill")+
  theme_classic()+
  scale_fill_discrete(labels=c('Open', 'Closed'), name="Valve gape")+
  labs(y="Valve gape")
...
#calculate proportion open/closed just for nov-march
```{r}
gapebinarypercentnov <- allgapemeta %>%
    filter(DateTime > '2022-11-01') %>% 
  group_by(Code) %>% #do calculations by oyster code
  drop_na(binary) %>% 
  group_by(Treatment,Site) %>% 
  count(binary) %>% 
...
gapebinarypercentnov=gapebinarypercentnov[gapebinarypercentnov$binary == '1', ]

ggplot(gapebinarypercentnov, aes(Treatment, percentopen))+
  geom_bar(position="dodge", stat="summary", fun="mean", fill=c("#999999", "#009E73", "chocolate4"), col="black")+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2, size = 0.75)+
  ylim(0,1)+
  theme_classic()+
...
ggsave(path=plots, "proportion_open_treatment_nov-mar.png", width=6, height=6)
```
...
##plot tides
julyaugusttides$DateTime=as.POSIXct(julyaugusttides$Date.Time, tz="UTC", format="%Y-%m-%d %H:%M:%S")
...
#Extra code \`\`\`{#r extra tide code} library(rnoaa) library(tidyverse)
library(ggplot2) library(suncalc) library(scales)

july.tides \<- as.data.frame(coops_search(station_name = 9410660,
begin_date = 20220701, end_date = 20210731, datum = "MLLW", product =
"predictions"))

march.tides \<- march.tides %\>% mutate(yyyy.mm.dd =
as.Date(predictions.t, tz = "EST")) dates \<-
unique(march.tides\$yyyy.mm.dd) sunlight.times \<- getSunlightTimes(date
= seq.Date(as.Date("2020/3/1"), as.Date("2020/3/31"), by = 1), lat =
39.5817, lon = -75.5883, tz = "EST")

for (i in seq_along(dates)) { plot \<- ggplot(subset(march.tides,
march.tides$yyyy.mm.dd==dates[i])) +  aes(x = predictions.t, y = predictions.v) +  geom_line(size = 1L, colour = "#0c4c8a") +  theme_bw() +  geom_vline(xintercept = sunlight.times$sunrise) +
geom_vline(xintercept =
sunlight.times$sunset) +  geom_vline(xintercept = sunlight.times$dawn,
linetype="dotted") + geom_vline(xintercept = sunlight.times\$dusk,
linetype="dotted") + ggtitle(dates[i]) + scale_x_datetime(labels =
date_format("%b %d %H:%M", tz = "EST")) print(plot) } S.M.2.1087 =
loadComboGape('S.M.2.1087') plotRawHallFunction(SN142)
plot2HallFunction(SN142) remove(SN142)

SN191 = loadComboGape('SN191') plot2HallFunction(SN191) remove(SN191)

SN118 = loadComboGape('SN118') #badO plot2HallFunction(SN118)
remove(SN118)

SN187 = loadComboGape('SN187') plot2HallFunction(SN187) remove(SN187)

SN152 = loadComboGape('SN152') \# The low hump at Jul 27 is shared with
SN176 plot2HallFunction(SN152) remove(SN152)

SN104 = loadComboGape('SN104') plot2HallFunction(SN104) remove(SN104)

SN176 = loadComboGape('SN176') \# The low hump around Jul 27 is shared
with SN152 plot2HallFunction(SN176) remove(SN176)

SN174 = loadComboGape('SN174') \# Bad sensor? plot2HallFunction(SN174)
remove(SN174)

SN130 = loadComboGape('SN130') plot2HallFunction(SN130) remove(SN130)

SN132 = loadComboGape('SN132') plot2HallFunction(SN132) remove(SN132)

SN123 = loadComboGape('SN123') \# Clean up the large outliers near start
and end plot2HallFunction(SN123) remove(SN123)

SN150 = loadComboGape('SN150') plot2HallFunction(SN150) remove(SN150)

SN138 = loadComboGape('SN138') plot2HallFunction(SN138) remove(SN138)

SN164 = loadComboGape('SN164') \# Weird change in opening limit around
Aug 6, like it could no longer open as far. SN164
plot2HallFunction(SN164) remove(SN164)

SN170 = loadComboGape('SN170') plot2HallFunction(SN170) remove(SN170)

SN115 = loadComboGape('SN115') #bad plot2HallFunction(SN115)
remove(SN115)

SN143 = loadComboGape('SN143') \# Some kind of malfunction
plot2HallFunction(SN143) remvove(SN143)

SN191 = loadComboGape('SN191') plot2HallFunction(SN191) remove(SN191)

SN125 = loadComboGape('SN125') \# Listed as dead, maybe why the upper
limit shifts down in the last few days? Around July 26
plot2HallFunction(SN125) remove(SN125)

SN197 = loadComboGape('SN197') plot2HallFunction(SN197) remove(SN197)

SN153 = loadComboGape('SN153') plot2HallFunction(SN153) remove(SN153)

SN171 = loadComboGape('SN171') \# Strange dip around Aug 1-5
plot2HallFunction(SN171) remove(SN171)

SN112 = loadComboGape('SN112') \# I think this one is saturating the
sensor when closed, magnet is too close, baseline closed value is
possibly spurious plot2HallFunction(SN112) remove(SN112)

SN193 = loadComboGape('SN193') \# Bad sensor plot2HallFunction(SN193)
remove(SN193)

SN118 = loadComboGape('SN118') \# Maybe died around Aug 9? Or did the
magnet fall off? plot2HallFunction(SN118) remove(SN118)

SN187 = loadComboGape('SN187') plot2HallFunction(SN187) remove(SN187)

SN161 = loadComboGape('SN161') plot2HallFunction(SN161) remove(SN161)

SN157 = loadComboGape('SN157') \# Bad sensor (magnetic field strength
saturated probably) plot2HallFunction(SN157) remove(SN157)

SN144 = loadComboGape('SN144') plot2HallFunction(SN144) remove(SN144)

SN159 = loadComboGape('SN159') \# Different looking pattern than most
others plot2HallFunction(SN159) remove(SN159)

SN148 = loadComboGape('SN148') plot2HallFunction(SN148) remove(SN148)

SN133 = loadComboGape('SN133') plot2HallFunction(SN133) remove(SN133)

SN104 = loadComboGape('SN104') plot2HallFunction(SN104) remove(SN104)

SN107 = loadComboGape('SN107') plot2HallFunction(SN107) remove(SN107)

SN111 = loadComboGape('SN111') plot2HallFunction(SN111) remove(SN111)

SN113 = loadComboGape('SN113') plot2HallFunction(SN113) remove(SN113)

SN114 = loadComboGape('SN114') plot2HallFunction(SN114) remove(SN114)

SN115 = loadComboGape('SN115') plot2HallFunction(SN115) remove(SN115)

SN120 = loadComboGape('SN120') plot2HallFunction(SN120) remove(SN120)

SN123 = loadComboGape('SN123') plot2HallFunction(SN123) remove(SN123)

SN130 = loadComboGape('SN130') plot2HallFunction(SN130) remove(SN130)

SN136 = loadComboGape('SN136') plot2HallFunction(SN136) remove(SN136)

SN138 = loadComboGape('SN138') plot2HallFunction(SN138) remove(SN138)

SN141 = loadComboGape('SN141') plot2HallFunction(SN141) remove(SN141)

SN143 = loadComboGape('SN143') plot2HallFunction(SN143) remove(SN143)

SN145 = loadComboGape('SN145') plot2HallFunction(SN145) remove(SN145)

SN147 = loadComboGape('SN147') plot2HallFunction(SN147) remove(SN147)

SN150 = loadComboGape('SN150') plot2HallFunction(SN150) remove(SN150)

SN152 = loadComboGape('SN152') plot2HallFunction(SN152) remove(SN152)

SN154 = loadComboGape('SN154') plot2HallFunction(SN154) remove(SN154)

SN156 = loadComboGape('SN156') plot2HallFunction(SN156) remove(SN156)

SN160 = loadComboGape('SN160') plot2HallFunction(SN160) remove(SN160)

SN164 = loadComboGape('SN164') plot2HallFunction(SN164) remove(SN164)

SN167 = loadComboGape('SN167') plot2HallFunction(SN167) remove(SN167)

SN170 = loadComboGape('SN170') plot2HallFunction(SN170) remove(SN170)

SN174 = loadComboGape('SN174') plot2HallFunction(SN174) remove(SN174)

SN176 = loadComboGape('SN176') plot2HallFunction(SN176) remove(SN176)

SN184 = loadComboGape('SN184') plot2HallFunction(SN184) remove(SN184)

SN194 = loadComboGape('SN194') plot2HallFunction(SN194) remove(SN194)

SN196 = loadComboGape('SN196') plot2HallFunction(SN196) remove(SN196)
...
#currentChannel = colnames(gapedat)[i] \# Locate the channel number from
currentChannel name #chNumRegex = regexpr('[0-9]+', currentChannel) \#
Extract the channel number #chNum = as.numeric(substr(currentChannel, \#
start=chNumRegex, \#
stop=(chNumRegex+attr(chNumRegex,'match.length')-1))) \# Generate the
regression fit for the relationship between curvature parameter \# 'c'
and the log-transform of 'b'. #chNum=1 #cRegress =
cRegressEstimate(gapeSN, chNum,gapecalibfile) \# Convert the Hall effect
values into estimated percent gape (0-100%) #gape.perc[,"hallpercent"] =
calcPercentGapeNLME(gapedat[,4], cRegress, \# percentileLim =
c(0.01,0.95))
...
#write.csv(gape.perc, \# file = paste0(gapeDir,SN107combogapefile), \#
row.names=FALSE)
...
#write.csv(LPLgape.perc.dec, \# file = paste0(gapeDir,LPL1minGapeFile)
\# row.names=FALSE) \`\`\`
